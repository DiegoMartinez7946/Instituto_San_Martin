...
func UserRoutes(router *mux.Router) (*mux.Router) {
	...
	router.HandleFunc("/user", middlewares.DbCheck(middlewares.ValidatedJWT(controllers.UpdateUser))).Methods("PUT")
	router.HandleFunc("/user/active", middlewares.DbCheck(middlewares.ValidatedJWT(controllers.ChangeActiveUser))).Methods("PUT")
